...
def consolidate_junctions(bed_gene_data, pre_consol_jct_df, outdir, skip_interm, prefix="tr"):
    ...
    if len(bed_gene_data['transcript_list']) == 1:
        ...
        key_gene = xcrpt_all[['gene_id','transcript_id','consolidation_transcript_id']]
        # Format output data
    ...
    # Check if more than one transcript present and all are monoexon (no junctions present)
        ...
        xcrpt_all['chr'] = bed_gene_data[xcrpt_all['transcript_id'][0]][0][0]
        xcrpt_all['strand'] = bed_gene_data[xcrpt_all['transcript_id'][0]][0][5]
        xcrpt_all['start'] = xcrpt_all['transcript_id'].map(lambda xcrpt: min([int(x[1])+1 for x in bed_gene_data[xcrpt]]))
        xcrpt_all['end'] = xcrpt_all['transcript_id'].map(lambda xcrpt: max([int(x[2]) for x in bed_gene_data[xcrpt]]))
        ...
        xcrpt_all['min_group_start'] = xcrpt_all.groupby('monoexon_group')['start'].transform('min')
        xcrpt_all['max_group_end'] = xcrpt_all.groupby('monoexon_group')['end'].transform('max')
        xcrpt_all['consolidation_transcript_id'] = prefix+"_"+gene_id+"_"+xcrpt_all['monoexon_group'].astype(int).map(str)
        key_gene = xcrpt_all[['gene_id','transcript_id','consolidation_transcript_id']]
        # Format output data
        consol_xcrpt = xcrpt_all.groupby('monoexon_group')[['gene_id','chr','strand','min_group_start','max_group_end']].first().reset_index()
        consol_xcrpt['consolidation_transcript_id'] = prefix+"_"+gene_id+"_"+consol_xcrpt['monoexon_group'].astype(int).map(str)
        consol_gene = consol_xcrpt[['chr','min_group_start','max_group_end','strand','gene_id','consolidation_transcript_id']]
        consol_gene.columns = consol_gene_cols
    # Gene contains at least one multiexon transcript and more than one transcript total
    else:
        logger.debug("Gene {} contains at least one multiexon transcript (more than 1 transcript present).", gene_id)
        # Get junction string for each transcript in the gene
        pre_consol_jct_df = pre_consol_jct_df.sort_values(['transcript_id','coords']).reset_index(drop=True)
        ...
        xcrpt_all['chr'] = bed_gene_data[xcrpt_all['transcript_id'][0]][0][0]
        xcrpt_all['strand'] = bed_gene_data[xcrpt_all['transcript_id'][0]][0][5]
        xcrpt_jct_w_mono = pd.merge(xcrpt_all, collapse_pre_jct, how='outer', on='transcript_id', validate="1:1")
        xcrpt_jct_w_mono['start'] = xcrpt_jct_w_mono['transcript_id'].map(lambda xcrpt: min([int(x[1])+1 for x in bed_gene_data[xcrpt]]))
        xcrpt_jct_w_mono['end'] = xcrpt_jct_w_mono['transcript_id'].map(lambda xcrpt: max([int(x[2]) for x in bed_gene_data[xcrpt]]))

        # Flag monoexon transcripts (no junctionID_order present)
        xcrpt_jct_w_mono['flag_monoexon_transcript'] = np.where(xcrpt_jct_w_mono['junctionID_order'].isna(),1,0)
        ...
        if xcrpt_jct_w_mono['flag_monoexon_transcript'].sum() > 0:
            # Loop over coordinates to make bins of overlapping monoexon transcripts
            xcrpt_jct_w_mono = xcrpt_jct_w_mono.sort_values(by=['start','end']).reset_index(drop=True)
            bin_num = 1
            bin_start =  xcrpt_jct_w_mono[xcrpt_jct_w_mono['flag_monoexon_transcript']==1]['start'].values[0]
            bin_end =  xcrpt_jct_w_mono[xcrpt_jct_w_mono['flag_monoexon_transcript']==1]['end'].values[0]
            xcrpt_jct_w_mono['monoexon_group'] = 0
            for index,row in xcrpt_jct_w_mono[xcrpt_jct_w_mono['flag_monoexon_transcript']==1].iterrows():
                if row['start'] >= bin_start and row['start'] <= bin_end:
                    ...
                    bin_end = row['end']
                    xcrpt_jct_w_mono.loc[index,'monoexon_group'] = bin_num
            logger.debug("Gene {} has {} monoexon transcript(s) that occupy {} nonoverlapping genomic region(s).", gene_id, xcrpt_jct_w_mono['flag_monoexon_transcript'].sum(),bin_num)
        ...
        xcrpt_jct_w_mono['transcriptID_cat'] = xcrpt_jct_w_mono.groupby(['junctionID_order'])['transcript_id'].transform(func=lambda x: "|".join(x))

        # Get minimum start and maximum end for longest 5' and 3' ends within each group
        #     with the same junctionID_order
        xcrpt_jct_w_mono['transcript_length'] = xcrpt_jct_w_mono['end'].astype(int) - xcrpt_jct_w_mono['start'].astype(int)
        xcrpt_jct_w_mono['min_group_start'] = xcrpt_jct_w_mono.groupby('junctionID_order')['start'].transform('min')
        xcrpt_jct_w_mono['max_group_end'] = xcrpt_jct_w_mono.groupby('junctionID_order')['end'].transform('max')
        ...
        longest_df['consol_transcript_length'] = longest_df['max_group_end'] - longest_df['min_group_start']
        ...
        for consol_transcript,transcript in key_gene.groupby('consolidation_transcript_id')['transcript_id'].first().iteritems():
            ...
            for row in bed_gene_data[transcript]:
                ...
                if num == 1:
                    ...
                    consol_gene = pd.concat([consol_gene,pd.DataFrame([[row[0],int(row[1])+1,longest_df[longest_df['consolidation_transcript_id']==consol_transcript]['consol_end'].values[0],row[5],gene_id,consol_transcript]],columns=consol_gene.columns)],ignore_index=True)
                else:
                    consol_gene = pd.concat([consol_gene,pd.DataFrame([[row[0],int(row[1])+1,int(row[2]),row[5],gene_id,consol_transcript]],columns=consol_gene.columns)],ignore_index=True)
                    consol_gene = consol_gene.sort_values(by=['transcript_id','start']).reset_index(drop=True)
    logger.debug("Gene {} has {} transcript(s) before consolidation and {} transcript(s) after consolidation.", gene_id, len(key_gene), key_gene['consolidation_transcript_id'].nunique())
