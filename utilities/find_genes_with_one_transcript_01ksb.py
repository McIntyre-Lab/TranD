...
def main():
    ...
    oneXscriptSet = set(geneDf['gene_id'][geneDf['transcript_id'].apply(
        lambda x: len(x) == 1)].to_list())
    print("Number of genes with one transcript: {}".format(len(set(oneXscriptSet))))
    print("Number of genes with more than one transcript (should match number of genes in TranD output): {}".format(
        len(uniqGeneSet) - len(oneXscriptSet)))
    ...
    outOneDf = pd.DataFrame({'geneID': list(oneXscriptSet)}).sort_values(
        by='geneID').reset_index(drop=True)

    geneLstFile = outDir + "/list_{}_uniq_gene_set.csv".format(prefix)
    oneListFile = outDir + "/list_{}_genes_w_one_xscript.csv".format(prefix)

    try:
        outGeneDf.to_csv(geneLstFile, index=False, header=False)
