...
def main():
    ...
    # refPath = "/nfshome/k.bankole/mnt/exasmb.rc.ufl.edu-blue/mcintyre/share/references/dmel_fb650/"
    refPath = "//exasmb.rc.ufl.edu/blue/mcintyre/share/references/dmel_fb650/"

    geneKeyFile = "{}/dmel650_2_dmel6_gffcompare_gene_key.csv".format(refPath)
    refAnnoFile = "{}/dmel-all-r6.50_subset_trans_spliced_gene.gtf".format(
        refPath)

    inKeyDf = pd.read_csv(geneKeyFile)
    inAnnoDf = trand.io.read_exon_data_from_file(refAnnoFile)

    keyDf = inKeyDf[['transcript_id', 'output_gene_id']].copy()

    annoDf = inAnnoDf.copy()

    annoDf = annoDf[['gene_id', 'transcript_id']].drop_duplicates()

    keyDf = inKeyDf[['transcript_id', 'output_gene_id']].copy()

    merge = pd.merge(annoDf, keyDf, how='outer',
                     on='transcript_id', indicator='merge_check')

    both = merge[merge['merge_check'] == 'both']

    both = both[['gene_id', 'output_gene_id', 'transcript_id']]

    bad = both[both['gene_id'] != both['output_gene_id']]

    badLst = bad[['gene_id', 'output_gene_id']].drop_duplicates()
    ...
    print("There are " + str(refGTFDf['transcript_id'].nunique()
                             ) + " unique transcripts in the reference.")
    print("There are " + str(unmapGTFDF['transcript_id'].nunique()
                             ) + " unmapped transcripts when self-mapping.")

    print("-> " + str(refGTFDf['transcript_id'].nunique() -
          unmapGTFDF['transcript_id'].nunique()) + " successfully mapped back onto the genome.")

    # Flag Unmapped Transcripts in Reference
    unmapMergeDf = pd.merge(refGTFDf, unmapGTFDF,
    ...
    unmapMergeDf['flagUnmap'] = unmapMergeDf['merge_check'].apply(
    ...
    refDf = refDf.groupby(['gene_id', 'transcript_id']).agg(
        "first").reset_index()[['gene_id', 'transcript_id', 'flagUnmap']]
    refXscript2Gene = refDf.rename(
        columns={'gene_id': 'ref_geneID', 'transcript_id': 'ref_transcriptID'})
    ...
    print("There are " + str(keyDf['transcript_id'].nunique()) +
          " unique reference transcripts in the minimap2 gtf/output from correct_gffcompare.")
    ...
    refAsgnMergeDf = pd.merge(refXscript2Gene, xscript2AsgnGn, on=[
                              'ref_transcriptID'], how='outer', indicator='merge_check')
    ...
    refAsgnMergeDf['asgn_geneID'] = refAsgnMergeDf.apply(lambda x: checkAsgnGID(
        x['merge_check'], x['flagUnmap'], x['asgn_geneID']), axis=1)
    ...
    noExonRow = refAsgnMergeDf[(
        refAsgnMergeDf['asgn_geneID'] == 'TR_NOT_IN_REF')]['ref_transcriptID'].unique()
    if noExonRow.size > 0:
        ...
        tracker = xscript2AsgnGn[~xscript2AsgnGn['ref_transcriptID'].isin(
            noExonRow)]
        print("Value for new number of unique transcripts in the corrected GTF: {}".format(
            tracker['ref_transcriptID'].nunique()))

        # Remove from output
        refAsgnMergeDf = refAsgnMergeDf[~refAsgnMergeDf['asgn_geneID'].str.contains(
            'TR_NOT_IN_REF')]
    ...
    notInFASTA = refAsgnMergeDf[refAsgnMergeDf['asgn_geneID']
                                == 'TR_NOT_IN_FASTA']['ref_transcriptID'].unique()
    if notInFASTA.size > 0:
        print("There are {} transcripts that not unmapped, but only appear in the reference annotation.".format(
            len(notInFASTA)))
        ...
        refAsgnMergeDf = refAsgnMergeDf[~refAsgnMergeDf['asgn_geneID'].str.contains(
            'TR_NOT_IN_FASTA')]
        print("Value for new number of unique reference transcripts: {}".format(
            refAsgnMergeDf['ref_transcriptID'].nunique()))

    # Verification
    if notInFASTA.size > 0:
        print("Verification: ")
        print("Number of unique transcripts in reference (excluding those not in the FASTA): {}".format(
            refGTFDf['transcript_id'].nunique() - len(notInFASTA)))
        print("Number of unique transcripts in output: {}".format(
            refAsgnMergeDf['ref_transcriptID'].nunique()))
        ...
        print("Number of unique transcripts in reference (excluding those not in the FASTA): {}".format(
            refGTFDf['transcript_id'].nunique()))
        print("Number of unique transcripts in output: {}".format(
            refAsgnMergeDf['ref_transcriptID'].nunique()))
    ...
    refAsgnMergeDf['flagDiffGeneID'] = refAsgnMergeDf.apply(
        lambda x: 1 if x['ref_geneID'] != x['asgn_geneID'] else 0, axis=1)

    # Output.
    outDf = refAsgnMergeDf.drop(columns='merge_check')
    outDf = outDf[['ref_transcriptID', 'ref_geneID',
                   'asgn_geneID', 'flagDiffGeneID', 'flagUnmap']]

    outFile = "{}/{}_gffcompare_correction_summary.csv".format(
        outDir, filePrefix)
    outDf.to_csv(outFile, index=False)

    # Output a list of transcripts to remove from the corrected GTF!
    removeFile = '{}/{}_remove_these_transcripts_from_corrected_gtf.txt'.format(
        outDir, filePrefix)
