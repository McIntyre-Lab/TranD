...
#                monoXscripts.to_csv("test.csv", index=False)
#                overlap = (monoXscripts["start"].shift(-1) < monoXscripts["end"]).cumsum()
                overlap = monoXscripts.groupby("gene_id").apply(lambda x: (x["start"].shift(-1) < x["end"]).cumsum()).reset_index().set_index("level_1")[0]
