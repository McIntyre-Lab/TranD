...
def getOptions():
        ...
        parser.add_argument(
                ...
                help="Add this argument to ignore genes when combining transcripts into UJCs."
                        "Example: \"tr_FBgn000000_1\" becomes \" tr_1 \""
        )
        ...
        #         dest="outGTF",
        #         action="store_true",
        #         help="Output a GTF with the UJCs as transcripts."
        # )
        ...
        if (strandCheck > 1).any():
                ...
                for gene in badStrand:
                        print("!!! WARNING: gene {} contains transcripts/exons on both strands - "
                              "removing from consolidation".format(gene))
...
def extractJunction(exonData):
        ...
        for xscriptID, group in xscriptGrps:
                ...
                if len(group) > 1:
                        ...
                        for row in sortedGrp.to_dict('records')[:-1]:
                                seqname = row['seqname']
                                strand = row['strand']
...
def createUJCDf(ujcDct, trPrefix, ignoreGene):
        ...
        # del (ujcDct?)
        # option to use geneID or ignore geneID, just remove groupby
        if len(multiExons) > 0:
...
if __name__ == '__main__':
        ...
        # masterUJCDct = None
        # masterUJCDct = storeVar(masterUJCDct, 'ujcDct.pickle')
