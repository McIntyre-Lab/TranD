...
def calculate_complexity(outdir,gtf_df,skip_plots,prefix=None):
    ...
    gtf_df['num_exon'] = 1
    gtf_df['exon_id'] = gtf_df['seqname'].map(str)+":"+gtf_df['start'].map(str)+":"+gtf_df['end'].map(str)+":"+gtf_df['strand'].map(str)
    transcriptDF = gtf_df.groupby('transcript_id').agg({'num_exon':'sum'}).reset_index()
    geneDF = gtf_df.groupby('gene_id').agg({'transcript_id':'nunique','num_exon':'sum','exon_id':'nunique'}).reset_index().rename(columns={'transcript_id':'num_transcript','exon_id':'num_uniq_exon'})
    ...
    counts.loc[0,'num_uniqExon'] = len(gtf_df[['seqname','start','end','strand']].drop_duplicates())
    ...
    if prefix is None:
        ...
        geneDF[['gene_id','num_uniq_exon']].to_csv("{}/unique_exon_per_gene.csv".format(outdir),index=False)
